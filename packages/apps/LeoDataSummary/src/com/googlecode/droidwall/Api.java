...
public final class Api {
	 ...
	 * @param uids
	 *            list of selected uids to allow or disallow (depending on the
	 *            working mode)
	 * @param showErrors
	...
	private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uids, boolean showErrors) {
		...
		try {
			...
			if (whitelist && wifi) {
				// When "white listing" Wi-fi, we need ensure that the dhcp and
				// wifi users are allowed
			...
			if (showErrors && code != 0) {
				...
				if (msg.indexOf("Couldn't find match `owner'") != -1 || msg.indexOf("no chain/target match") != -1) {
					alert(ctx,
							"Error applying iptables rules./nExit code: "
									+ code
									+ "/n/n"
	...
	public static DroidApp[] getApps(Context ctx) {
		...
		try {
			...
			for (final ApplicationInfo apinfo : installed) {
				...
				// Check for the tethering application (which causes conflicts
				// with Droid Wall)
			...
			final DroidApp special[] = { new DroidApp(android.os.Process.getUidForName("root"), "root", "(Applications running as root)", false),
	...
	public static boolean hasRootAccess(Context ctx) {
		...
		alert(ctx, "Could not acquire root access./n" + "You need a rooted phone to run Droid Wall./n/n"
	...
	/**
	 * Runs a script as root (multiple commands separated by "/n") with a
	...
	public static final class DroidApp {
		...
		int uid;
		/**
		 * application user name (Android actually uses a package name to
		 * identify)
		 */
		String username;
	...
	private static final class ScriptRunner extends Thread {
		...
		public void run() {
			try {
				...
				if (res != null)
					res.append("/nOperation timed-out");
			} catch (Exception ex) {
				if (res != null)
					res.append("/n" + ex);
			} finally {
