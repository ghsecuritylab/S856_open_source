		...
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	bat_voltage = bat_voltage + bat_temp_vol_comp;
	ret = qpnp_adc_map_temp_voltage(
			adcmap_btm_threshold,
			ARRAY_SIZE(adcmap_btm_threshold),
			bat_voltage,
			&adc_chan_result->physical);
	pr_debug("battery temp is %d,voltage is %d,voltage compensation is %d\n",(int)adc_chan_result->physical,(int)bat_voltage,bat_temp_vol_comp);
	return ret;
}
