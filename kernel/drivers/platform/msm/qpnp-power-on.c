...
#define QPNP_PON_S3_ENABLE(base)                (base + 0x74)
...
#define QPNP_PON_S3_RESET_EN                    BIT(0)
...
static int __devinit qpnp_pon_config_init(struct qpnp_pon *pon)
{
	...
	/* disable S3 RESIN Reset */
	rc = qpnp_pon_masked_write(pon, QPNP_PON_S3_ENABLE(pon->base),
			 QPNP_PON_S3_RESET_EN, 0);
	if (rc) {
		dev_err(&pon->spmi->dev, "Unable to configure S3 enable\n");
		return rc;
	}
