...
struct qpnp_chg_chip	*tmp_chip = NULL;
...
qpnp_chg_charge_en(struct qpnp_chg_chip *chip, int enable)
{
	if (chip->insertion_ocv_uv == 0 && enable) {
		printk("Battery not present, don't skipping\n");
	//	return 0;
	}
...
qpnp_chg_force_run_on_batt(struct qpnp_chg_chip *chip, int disable)
{
	...
	if (!qpnp_chg_is_batt_present(chip))
		printk("force Battery not present, don't skipping\n");
...
static void qpnp_usb_event_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, usb_event_work);
	if(!qpnp_chg_is_usb_chg_plugged_in(chip))
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			schedule_delayed_work(&chip->usb_event_work,0);
		} else {
...
static int
get_chg_status(struct qpnp_chg_chip *chip)
{
	int rc;
	u8 chgr_sts, bat_if_sts;
...
{
	union power_supply_propval ret = {0,};
	int battery_status, soc, charger_in;
	...
	if (chip->bms_psy) {
		chip->bms_psy->get_property(chip->bms_psy,
				POWER_SUPPLY_PROP_CAPACITY, &ret);
		soc = ret.intval;
		battery_status = get_chg_status(chip);
//		chip->bms_psy->get_property(chip->bms_psy,
...
//				&& bms_status != POWER_SUPPLY_STATUS_CHARGING
			...
			printk("resuming charging at %d%% soc\n", soc);
		...
		if ((qpnp_chg_is_usb_chg_plugged_in(chip) ||
			qpnp_chg_is_dc_chg_plugged_in(chip)) && (chip->chg_done && !full_capacity) && !chip->bat_is_warm) {
...
#define BATT_CON_R 15
int bat_temp_vol_comp = 0;
static int
get_prop_batt_temp(struct qpnp_chg_chip *chip)
{
	int rc = 0;
	struct qpnp_vadc_result results;
	int current_now = get_prop_current_now(chip)/1000;
	int batt_status = get_prop_batt_status(chip);
	bat_temp_vol_comp = current_now*BATT_CON_R/1000;
/*1.when charger is plugged out ,but battery current is negative, don't do compensation;
 *2.when bat_temp_vol_comp is too large, don't do compensation
 * */
	if((batt_status == POWER_SUPPLY_STATUS_DISCHARGING && bat_temp_vol_comp < 0) || abs(bat_temp_vol_comp) > 50)
		bat_temp_vol_comp = 0;
...
qpnp_batt_external_power_changed(struct power_supply *psy)
{
	...
	if(chip->ac_online == 1)
		gpio_set_value(chip->ext_ovp_en_gpio, 1);//enable ext ovp
	else
		gpio_set_value(chip->ext_ovp_en_gpio, 0);//disable ext ovp
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_IBAT_MAX:
		val->intval = chip->max_bat_chg_current;
		break;
	default:
		return -EINVAL;
	}
...
qpnp_temp_mon_work(struct work_struct *work)
{
	...
	if(temp > 0 && temp  < chip->cool_bat_decidegc){
		bat_cool = true;
		bat_warm = false;
		if(batt_voltage > 4150)
		{
			dec_cur_bat_cool = true;
			chip->cool_bat_chg_ma = 300;
		}else
		{
			dec_cur_bat_cool = false;
			chip->cool_bat_chg_ma = 500;
		}
	}else if(temp > chip->cool_bat_decidegc && temp < chip->warm_bat_decidegc){
		bat_cool = false;
		bat_warm = false;
	}else if(temp > chip->warm_bat_decidegc && temp < 500){
		bat_warm = true;
		bat_cool = false;
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		...
		break;
	case POWER_SUPPLY_PROP_IBAT_MAX:
		chip->max_bat_chg_current = val->intval;
		qpnp_chg_set_appropriate_battery_current(chip);
		break;
	default:
		return -EINVAL;
...
qpnp_charger_read_dt_props(struct qpnp_chg_chip *chip)
{
	...
	chip->ext_ovp_en_gpio = of_get_named_gpio_flags(chip->spmi->dev.of_node,
			"qcom,ext_ovp_en_gpio", 0, NULL);
	if (rc)
...
void ext_ovp_en_init_func(struct qpnp_chg_chip* chip)
{
	int retval = -1;
	retval = gpio_request(chip->ext_ovp_en_gpio,
			"ext_ovp_en_gpio");
	if (retval) {
		printk("ext_ovp_en_gpio request error %d\n",retval);
	}

	retval = gpio_direction_output(chip->ext_ovp_en_gpio, 1);
	if (retval) {
		printk("ext_ovp_en_gpio set output error %d\n",retval);
	}

	gpio_set_value(chip->ext_ovp_en_gpio, 0);

}
static char *ac_pm_power_supplied_to[] = {
	"battery",
};
static enum power_supply_property ac_pm_power_props_ac[] = {
	POWER_SUPPLY_PROP_ONLINE,
};
static int ac_power_get_property_ac(struct power_supply *psy,
				  enum power_supply_property psp,
				  union power_supply_propval *val)
{
	if(tmp_chip == NULL)
	...
	case POWER_SUPPLY_PROP_ONLINE:
		if(is_charger_plug_in() == 0)
			tmp_chip->ac_online = 0;
		val->intval = tmp_chip->ac_online;
		break;
	default:
		return -EINVAL;
	}
...
static int ac_power_set_property_ac(struct power_supply *psy,
				  enum power_supply_property psp,
				  const union power_supply_propval *val)
{
	if(tmp_chip == NULL)
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		tmp_chip->ac_online = val->intval;
		break;
	default:
		return -EINVAL;
	}

	power_supply_changed(&tmp_chip->ac_psy);
	return 0;
}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	if (chip->bat_if_base) {
		...
		chip->ac_psy.supplied_to = ac_pm_power_supplied_to;
		chip->ac_psy.num_supplicants = ARRAY_SIZE(
						ac_pm_power_supplied_to);
		chip->ac_psy.properties = ac_pm_power_props_ac;
		chip->ac_psy.num_properties =
					ARRAY_SIZE(ac_pm_power_props_ac);
		chip->ac_psy.get_property = ac_power_get_property_ac;
		chip->ac_psy.set_property = ac_power_set_property_ac;

		rc = power_supply_register(chip->dev, &chip->ac_psy);
		if (rc < 0) {
			pr_err("%s:power_supply_register ac failed\n",
						__func__);
			goto fail_chg_enable;
		}
	}
	...
	INIT_DELAYED_WORK(&chip->usb_event_work, qpnp_usb_event_work);
