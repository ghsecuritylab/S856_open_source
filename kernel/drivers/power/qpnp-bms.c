							...
							int soc, int batt_temp)
{
	...
	if((ocv_est_uv < 3450000 && battery_voltage < 3450) || battery_voltage < 3000)
	{
		low_vol_count++;
...
static void configure_vbat_monitor_low(struct qpnp_bms_chip *chip)
{
	...
	if (chip->vbat_monitor_params.state_request
		...
		if (!wake_lock_active(&chip->low_voltage_wake_lock)) {
			...
			wake_lock(&chip->low_voltage_wake_lock);
			mutex_unlock(&chip->vbat_monitor_mutex);//unlock the vbat_monitor_mutex first,because the cancel_delayed_work_sync will result in the soc caluclated work to request this mutex
			cancel_delayed_work_sync(
					&chip->calculate_soc_delayed_work);
			mutex_lock(&chip->vbat_monitor_mutex);//lock again
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	if (chip->batt_type == BATT_LENOVO_4V35_2500MAH) {
		batt_data = &lenovo_2500_data;
