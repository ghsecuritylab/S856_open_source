...
static int test_task_flag(struct task_struct *p, int flag)
{
    ...
    /* use for each thread to replace buggy while_each_thread() */
	for_each_thread(p,t) {
		task_lock(t);
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
    ...
    if (gfp_zone(sc->gfp_mask) == ZONE_NORMAL) {
        lowmem_print(4, "lmk_shrink: low mem request, scale it\n");
        other_free *= total_low_ratio;
        other_file -= (si.freeswap * 4) / 5;
        other_file *= total_low_ratio;
        other_file += (si.freeswap * 4) / 5;
        }
    else {
        lowmem_print(4, "lmk_shrink: low mem request\n");
        }
#endif
...
void could_cswap(void)
{
	...
	if (atomic_long_read(&nr_swap_pages) < (long)minimum_freeswap_pages)
		return;
...
static int __init lowmem_init(void)
{
    ...
    printk(KERN_ALERT "[lmk] total ratio: %d", total_low_ratio);
#endif
#ifdef CONFIG_ZRAM_FOR_ANDROID
