...
static int msm_otg_notify_power_supply(struct msm_otg *motg, unsigned mA)
{
    ...
    struct power_supply *oth_psy = NULL;
	if (!psy) {
	...
	if (motg->cur_power == 0 && mA > 2) {
		/* Enable charging */
		if (power_supply_set_online(tmp_psy, true))
			goto psy_error;
		...
		if (power_supply_set_current_limit(tmp_psy, 0))
			goto psy_error;
	} else {
		if (power_supply_set_online(tmp_psy, true))
			goto psy_error;
		if (power_supply_set_online(oth_psy, false))
			goto psy_error;
...
bool chg_type_det_err = false;
extern int is_charger_plug_in(void);
static void msm_chg_type_redet_work(struct work_struct *w)
{
	if(chg_type_det_err && is_charger_plug_in() && ac_psy != NULL)
	{
		printk("usb don't enumeration, set chg type as AC\n");
		power_supply_set_online(ac_psy, true);
...
static void msm_otg_notify_charger(struct msm_otg *motg, unsigned mA)
{
	...
	{
		mA = 950;
		chg_type_det_err = true;
		__cancel_delayed_work(&motg->chg_type_redet_work);
		schedule_delayed_work(&motg->chg_type_redet_work,msecs_to_jiffies(3000));
	}else
		chg_type_det_err = false;
	...
	dev_info(motg->phy.dev, "Avail curr from USB = %u,charge_type is %d\n", mA,motg->chg_type);
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		if(ac_psy != NULL)
			ac_psy->get_property(ac_psy,POWER_SUPPLY_PROP_ONLINE, val);
		val->intval = val->intval || motg->online;
		pr_debug("%s:present is %d\n",__func__,val->intval);
		break;
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&motg->chg_type_redet_work,msm_chg_type_redet_work);
