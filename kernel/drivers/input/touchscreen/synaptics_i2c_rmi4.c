...
if(synaptics_debug)
     printk("Button %d,status %d.\n",f1a->button_map[button], status);
		...
		if(synaptics_debug)
			printk("input_report_key before_2d_status[%d] is %d,while_2d_status[%d] is %d\n",button,before_2d_status[button],button,while_2d_status[button]);
			input_report_key(rmi4_data->input_dev,
					f1a->button_map[button],
					status);
		} else {
			if (before_2d_status[button] == 1) {
				before_2d_status[button] = 0;
		if(synaptics_debug)
			printk("input_report_key before_2d_status[%d] is %d\n",button,before_2d_status[button]);
				input_report_key(rmi4_data->input_dev,
						f1a->button_map[button],
						status);
			} else {
              if(synaptics_debug)
 				printk("not input report !!\n");
				if (status == 1)
