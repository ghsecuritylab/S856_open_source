...
extern char lenovo_panel_name[];
static ssize_t mdss_dsi_show_panel_name(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
	ret = sprintf(buf, "%s\n", lenovo_panel_name);
	return ret;
...
static ssize_t mdss_dsi_lcd_onoff_get(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
...
static ssize_t mdss_dsi_lcd_onoff_set(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t n)
{
	unsigned desired_lcd_onoff;
	int ret;

	ret = sscanf(buf, "%u", &desired_lcd_onoff);
	if (desired_lcd_onoff < 0 || desired_lcd_onoff > 1) {
		dev_err(dev, "invalid lcd_onoff value\n");
		return -EINVAL;
	}
	
	lcd_onoff_control(desired_lcd_onoff);
	return n;
}

static ssize_t mdss_dsi_cabc_onoff_get(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%02x\n", mdss_dsi_get_cabc_state());
}
static ssize_t mdss_dsi_cabc_onoff_set(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t n)
{
	unsigned desired_cabc_onoff;
	int ret;

	ret = sscanf(buf, "%u", &desired_cabc_onoff);
	if (desired_cabc_onoff < 0) {
		dev_err(dev, "invalid lcd_onoff value\n");
		return -EINVAL;
	}
	
	mdss_dsi_panel_cabc_onoff(desired_cabc_onoff);
	return n;
}

static ssize_t mdss_dsi_ce_onoff_get(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%02x\n", mdss_dsi_get_ce_state());
}
static ssize_t mdss_dsi_ce_onoff_set(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t n)
{
	unsigned desired_ce_onoff;
	int ret;

	ret = sscanf(buf, "%u", &desired_ce_onoff);
	if (desired_ce_onoff < 0) {
		dev_err(dev, "invalid lcd_onoff value\n");
		return -EINVAL;
	}
	
	mdss_dsi_panel_ce_onoff(desired_ce_onoff);
	return n;
}

static ssize_t mdss_dsi_dimming_onoff_get(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%02x\n", mdss_dsi_get_dimming_state());
}
static ssize_t mdss_dsi_dimming_onoff_set(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t n)
{
	unsigned desired_dimming_onoff;
	int ret;

	ret = sscanf(buf, "%u", &desired_dimming_onoff);
	if (desired_dimming_onoff < 0) {
		dev_err(dev, "invalid lcd_onoff value\n");
		return -EINVAL;
	}

	mdss_dsi_panel_dimming_onoff(desired_dimming_onoff);
	return n;
}
...
static DEVICE_ATTR(lcd_name, S_IRUGO, mdss_dsi_show_panel_name, NULL);
static DEVICE_ATTR(lcd_onoff, S_IRUGO | S_IWUSR | S_IWGRP, mdss_dsi_lcd_onoff_get, mdss_dsi_lcd_onoff_set);
static DEVICE_ATTR(ce_onoff, S_IRUGO | S_IWUSR | S_IWGRP, mdss_dsi_ce_onoff_get, mdss_dsi_ce_onoff_set);
static DEVICE_ATTR(cabc_onoff, S_IRUGO | S_IWUSR | S_IWGRP, mdss_dsi_cabc_onoff_get, mdss_dsi_cabc_onoff_set);
static DEVICE_ATTR(dimming_onoff, S_IRUGO | S_IWUSR | S_IWGRP, mdss_dsi_dimming_onoff_get, mdss_dsi_dimming_onoff_set);
...
static int mdss_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	pr_info("%s: ====== pid=%u == name= %s\n", __func__,current->tgid, comm);
