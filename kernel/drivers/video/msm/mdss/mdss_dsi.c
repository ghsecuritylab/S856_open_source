...
#define ESD_OPRATION
#ifdef ESD_OPRATION
#define ESD_DELAY_TIME 500
struct delayed_work lcd_esd_work;
struct timer_list esd_te_timer;
				  ...
				  int event, void *arg)
{
	...
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	pr_info("%s+:event=%d ######################## pid=%u, process: %s\n", __func__, event, current->tgid, comm);
	...
	case MDSS_EVENT_PANEL_ON:
		...
		pdata_esd = pdata;
#ifdef ESD_OPRATION
		//schedule_delayed_work(&lcd_esd_work, ESD_DELAY_TIME);
		init_timer(&esd_te_timer);
		esd_te_timer.function =  te_timer_handle;
		enable_irq(ctrl_pdata->te_irq);
#endif
		break;
	case MDSS_EVENT_BLANK:
#ifdef ESD_OPRATION
		//rc = cancel_delayed_work_sync(&lcd_esd_work);
		//if (!rc)
		//	pr_warn("%s: !!!!ESD delayed work not in pending state!!!!\n", __func__);
		disable_irq(ctrl_pdata->te_irq);
		del_timer(&esd_te_timer);
#endif
...
static int __devinit mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: Dsi Ctrl->%d initialized. nodename=%s\n", __func__, index, dsi_pan_node->name);
	return 0;
...
{
	pr_info("mdss: ###%s\n", __func__);
	schedule_delayed_work(&lcd_esd_work, 0);
}		
static void lcd_esd_handle(struct work_struct *work)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	if (pdata_esd == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
...
#if 1  //ESD:by checking te interrput
	mdss_dsi_event_handler(pdata_esd, MDSS_EVENT_BLANK, 0);
	mdss_dsi_event_handler(pdata_esd, MDSS_EVENT_PANEL_OFF, 0);
	msleep(10);
	mdss_dsi_event_handler(pdata_esd, MDSS_EVENT_UNBLANK, 0);
	mdss_dsi_event_handler(pdata_esd, MDSS_EVENT_PANEL_ON, 0);
#else  //ESD:by checking lcd status
	char esd_status[8];
	mdss_dsi_panel_cmd_read(ctrl_pdata, 0x0a, 0x0, NULL, esd_status, 4);
	if (!(ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT))
	{
		pr_info("###%s: lcd already power off 2\n", __func__);
		return;
	}
	if(esd_status[0]!=0x9c)
	{
		pr_info("###%s: esd_status=%x %x\n", __func__, esd_status[0], esd_status[1]);
		mdss_dsi_event_handler(pdata_esd, MDSS_EVENT_BLANK, 0);
		mdss_dsi_event_handler(pdata_esd, MDSS_EVENT_PANEL_OFF, 0);
		msleep(10);
		mdss_dsi_event_handler(pdata_esd, MDSS_EVENT_UNBLANK, 0);
...
{
	static int te_irq_cnt = 0;
	if((te_irq_cnt++ & 0x3f) == 0)
	{
		mod_timer(&esd_te_timer, 2 * HZ + jiffies);
		//pr_info("mdss: ###%s\n", __func__);
	}
	return IRQ_HANDLED;
}
void check_lcd_ID(void)
{
	char lcd_ID[8];
	char dcs_cmd_page[6] = {0xF0, 0x55, 0xAA, 0x52, 0x08, 0x01}; /* DTYPE_DCS_READ */
	struct dsi_cmd_desc dcs_page_cmd = {
		{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(dcs_cmd_page)},
		dcs_cmd_page
	};
	struct dsi_panel_cmds pagecmds;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	if (pdata_esd == NULL) {
		pr_err("mdss: %s: Invalid input data\n", __func__);
		return;
	}
	ctrl_pdata = container_of(pdata_esd, struct mdss_dsi_ctrl_pdata,panel_data);
	dcs_cmd_page[5] = 1;
	pagecmds.cmds = &dcs_page_cmd;
	pagecmds.cmd_cnt = 1;
	mdss_dsi_panel_cmds_send(ctrl_pdata, &pagecmds);
	mdss_dsi_panel_cmd_read(ctrl_pdata, 0xc5, 0x0, NULL, lcd_ID, 4);
	pr_info("###%s: 0xc5=%x %x %x %x\n", __func__, lcd_ID[0], lcd_ID[1], lcd_ID[2], lcd_ID[3]);
}
#endif

void lcd_onoff_control(int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	if (pdata_esd == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	pr_info("mdss %s: pclk=%d, bclk=%d, mode_gpio=%d disp_en_gpio=%d rst_gpio=%d disp_te_gpio=%d\n", __func__,
			ctrl_pdata->pclk_rate, ctrl_pdata->byte_clk_rate,
			ctrl_pdata->mode_gpio,ctrl_pdata->disp_en_gpio,ctrl_pdata->rst_gpio,ctrl_pdata->disp_te_gpio);
...
#ifdef ESD_OPRATION
	INIT_DELAYED_WORK(&lcd_esd_work, lcd_esd_handle);
	...
	if(rc)
		pr_err("mdss %s: failed to request TE irq %d", __func__, ctrl_pdata->te_irq);
	disable_irq(ctrl_pdata->te_irq);
#endif

	pr_info("mdss %s: Panel data initialized te_irq=%d\n", __func__, ctrl_pdata->te_irq);
	return 0;
}
