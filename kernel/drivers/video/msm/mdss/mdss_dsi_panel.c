...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	{
		pr_info("%s: level=%d, failed. lcd already poweroff\n", __func__, level);
		return;
	}
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		pr_debug("%s:%d, disp_en_gpio line not configured\n",
			   __func__, __LINE__);
	}
	...
	pr_info("%s: enable = %d   gpio52=%d\n", __func__, enable, gpio_get_value(52));
	...
	if (enable) {
		...
		rc = isl98607_set_mode(BIAS_MODE_5P5V);
		msleep(50);
		...
		msleep(20); //rst->initcode, panel need 20ms delay
	} else {
		gpio_set_value((ctrl_pdata->rst_gpio), 0);
		rc = isl98607_set_mode(BIAS_MODE_SLEEP);
		msleep(1);
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	mipi  = &pdata->panel_info.mipi;
	mdss_lcd_parse_file_cmds(&ctrl->on_cmds);

	pr_info("%s: ctrl=%p ndx=%d, on_cmds_cnt=%d\n", __func__, ctrl, ctrl->ndx, ctrl->on_cmds.cmd_cnt);
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	pr_info("%s: ctrl=%p ndx=%d, off_cmds_cnt=%d\n", __func__, ctrl, ctrl->ndx, ctrl->off_cmds.cmd_cnt);
...
//#endif
	...
	pr_info("%s: porch:%d:%d:%d:%d:%d:%d, power_stop:%d:%d:%d:%d:%d, lp11_init:%d\n", __func__, pinfo->lcdc.h_front_porch, pinfo->lcdc.h_back_porch, pinfo->lcdc.h_pulse_width, pinfo->lcdc.v_front_porch, pinfo->lcdc.v_back_porch, pinfo->lcdc.v_pulse_width, pinfo->mipi.hfp_power_stop, pinfo->mipi.hsa_power_stop, pinfo->mipi.hbp_power_stop, pinfo->mipi.bllp_power_stop, pinfo->mipi.eof_bllp_power_stop, pinfo->mipi.lp11_init);
...
char lenovo_panel_name[MDSS_MAX_PANEL_LEN];
	...
	bool cmd_cfg_cont_splash)
{
	...
	pr_info("%s:  node->name=%s, cmd_cfg_cont_splash=%d\n", __func__, node->name, cmd_cfg_cont_splash);
	...
	if(strstr(panel_name,"boe"))
		strcpy(lenovo_panel_name,"NT35521_BTL557212_BOE_1280_720_5.5");
	else
		strcpy(lenovo_panel_name,"NT35521_TM055JVSP12_TIANMA_1280_720_5.5");
