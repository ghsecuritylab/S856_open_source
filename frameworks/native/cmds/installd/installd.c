    ...
    if (version == 0) {
        ...
        // modified by zhongwei1@lenovo.com end
            ...
            if (access(media_tmp_dir, F_OK) == 0) {
                //ALOGD("just test normal!\n");
                if (fs_write_atomic_int(log_path, 1) == -1) {
                    ALOGE("Failed to save flag to %s: %s", log_path, strerror(errno));
                    goto fail;
                }
            ...
            while ((dirent = readdir(dir))) {
                if (dirent->d_type == DT_DIR) {
                    ...
                    if (fs_prepare_dir(user_media_dir, 0770, AID_MEDIA_RW, AID_MEDIA_RW) == -1) {
                        ALOGE("Failed to ensure the exist media path: %s", strerror(errno));
                        goto fail;
                    }
                }
            }
...
fail:
    // should write back the version to avoid exception betwen 1 and 2
