...
//Tony add for read hci data from Bluedroid through socket port 4330
#define NET_PORTNUM	4330
static char net_addr[16] = "127.0.0.1";
static int net_port = -1;
...
//Tony add for read hci data from Bluedroid through socket port 4330
static int net_connect(int port)
{
    socklen_t           clilen;
    ...
    if( inet_pton(AF_INET, net_addr, &servaddr.sin_addr) <= 0){
...
int main(int argc, char *argv[])
{
	...
	while ((opt=getopt_long(argc, argv, "i:l:p:m:w:r:d:taxXRC:H:O:P:D:A:YZ46nhv", main_options, NULL)) != -1) {
		switch(opt) {
		case 'i':
