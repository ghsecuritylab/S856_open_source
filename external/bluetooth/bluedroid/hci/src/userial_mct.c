...
//qiaoshan added for SHELLROW-982
static volatile uint8_t userial_thread_created = 0;
//qiaoshan added for SHELLROW-1712
...
void userial_mct_close(void)
{
    ...
    //qiaoshan added for SHELLROW-982
    /* if the thread is created wait for thread to run */
    if(userial_thread_created)
    {
        ALOGE("userial_close userial_thread_created userial_running is %d ", userial_running);
        while(!userial_running)
        {
            /* sleep for 20 msec */
            usleep(20000);
