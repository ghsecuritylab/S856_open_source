...
int logging_cfg_onoff(char *p_conf_name, char *p_conf_value)
{
    //Add for OfflineLog
    char val[256];
    property_get("persist.bt.btsnoop", val, "unknow");
    if (strcmp(val, "true") == 0) {
        ALOGD("persist.bt.btsnoop = true");
        hci_logging_enabled = TRUE;
    } else if (strcmp(val, "false") == 0) {
        ALOGD("persist.bt.btsnoop = false");
        hci_logging_enabled = FALSE;
    } else {
        ALOGD("persist.bt.btsnoop not set, using bt_stack.conf");
...
int logging_set_filepath(char *p_conf_name, char *p_conf_value)
{
    ...
    property_get("persist.bt.btsnoop", val, "unknow");
    if (strcmp(val, "true") == 0) {
        property_get("persist.bt.filepath", val, NULL);
        if (strlen(val)) {
            if ((lstat(val, &st) == 0) && S_ISDIR(st.st_mode)) {
                //path is dir, set btsnoop filepath with time info.
                if (val[strlen(val)-1] == '/')
                    sprintf(hci_logfile, "%sbtsnoop_hci-%04d%02d%02d-%02d%02d%02d.log", val,
                        ptm->tm_year+1900, ptm->tm_mon+1, ptm->tm_mday,
                        ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
                else
                    sprintf(hci_logfile, "%s/btsnoop_hci-%04d%02d%02d-%02d%02d%02d.log", val,
                    ptm->tm_year+1900, ptm->tm_mon+1, ptm->tm_mday,
                    ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
            } else {
                //path is not dir, use it as a btsnoop filepath
                sprintf(hci_logfile, "%s", val);
            }
        } else {
            //null filepath, use default
//            sprintf(hci_logfile, "%s", "/sdcard/btsnoop_hci.log");
            sprintf(hci_logfile, "/data/local/log/aplog/btsnoop_hci-%04d%02d%02d-%02d%02d%02d.log",
                ptm->tm_year+1900, ptm->tm_mon+1, ptm->tm_mday,
                ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
        }
        ALOGD("persist.bt.btsnoop = true, save to %s", hci_logfile);
    } else if (strcmp(val, "false") == 0) {
        ALOGD("persist.bt.btsnoop = false");
    } else {
        ALOGD("persist.bt.btsnoop not set, using bt_stack.conf BtSnoopFileName");
