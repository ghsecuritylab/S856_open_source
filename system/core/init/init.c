...
static void import_kernel_nv(char *name, int for_emulator)
{
    ...
    *value++ = 0;
    ...
    } else if (!strcmp(name,"testmode")) {
        strlcpy(testmode, value, sizeof(testmode));
...
int main(int argc, char **argv)
{
    ...
    if(testmode[0] == '1')
	snprintf(tmp, sizeof(tmp), "/init.testmode.rc");
    else
	snprintf(tmp, sizeof(tmp), "/init.rc");

    ERROR("parse config file: %s\n", tmp);
