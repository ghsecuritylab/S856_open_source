...
{
/*[lenovo_fw,zhouli4,lesafe-adb] ,Decription/"Refer to lenovo_fw.txt" , add*/
#if !SPEEDUP
/*[lenovo_fw,zhouli4,lesafe-adb] ,end*/
    int n;
    ...
    D("[ done fd=%d ]\n", h->fd);
    return 0;
/*[lenovo_fw,zhouli4,lesafe-adb] ,Decription/"Refer to lenovo_fw.txt" */
#else
	//if readsize > 4096,kernel return EINVAL
	//see kernel/drivers/usb/gadget/f_adb.c usb_read
    ...
    while(len > 0)
    {
        xlen = (len > 4096)? 4096:len;
        D("[ true read %d %d ]\n",h->fd,xlen);
        n = adb_read(h->fd,data,xlen);
        if(n!= xlen)
        {
