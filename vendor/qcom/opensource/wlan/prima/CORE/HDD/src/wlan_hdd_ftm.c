...
//tpAniSirGlobal pMac;
static tPttMsgbuffer *pMsgBuf=NULL;
...
static VOS_STATUS wlan_ftm_priv_set_wfgain(hdd_adapter_t *pAdapter,v_S15_t dGain,v_U16_t rfGain)
{
  ...
  hdd_context_t *pHddCtx = (hdd_context_t *)pAdapter->pHddCtx;
	...
	/* do not allow to change setting when wf gen is enabled */
	...
	wlan_ftm_priv_start_stop_tx_pktgen(pAdapter,1);
...
static VOS_STATUS wlan_ftm_priv_cw_rf_gen(hdd_adapter_t *pAdapter,v_U16_t startStop)
{
	...
	if (startStop == 1)
	{
		//tWaveformSample wfmBuffer[MAX_TEST_WAVEFORM_SAMPLES];
		tANI_U16 numSamples = 256;
		tANI_BOOLEAN clk80 = TRUE;
		int n = 0;
		v_BYTE_t msgT[4] = {0xff,0x00,0x00,0x00};
		init_completion(&pHddCtx->ftm.ftm_comp_var);
		pMsgBuf->msgId = PTT_MSG_SET_WAVEFORM_RF;
		...
		for(n=0;n<128*2;n++)
		{
			memcpy((v_BYTE_t*)pMsgBody->SetWaveformRF.waveform+n*4,msgT,4);
		}
		...
		init_completion(&pHddCtx->ftm.ftm_comp_var);
		pMsgBuf->msgId = PTT_MSG_START_WAVEFORM_RF;
		pMsgBuf->msgBodyLength = sizeof(tMsgPttStartWaveformRF) + PTT_HEADER_LENGTH;
		pMsgBody = &pMsgBuf->msgBody;
		pMsgBody->StartWaveformRF.startIndex = 0;
		pMsgBody->StartWaveformRF.numSamples = 256;
	...
	{
		init_completion(&pHddCtx->ftm.ftm_comp_var);
		pMsgBuf->msgId = PTT_MSG_STOP_WAVEFORM_RF;
		pMsgBuf->msgBodyLength = sizeof(tMsgPttStopWaveformRF) + PTT_HEADER_LENGTH;
		pMsgBody = &pMsgBuf->msgBody;
...
#define PHY_DOUBLE_CHANNEL_CENTERED 2 
static unsigned int get_primary_channel(unsigned int center_channel,unsigned int cb)
{
    ...
    switch(cb)
    {
        ...
        case PHY_DOUBLE_CHANNEL_LOW_PRIMARY:
            primary_channel = center_channel - 2;
            break;
		...
		case PHY_QUADRUPLE_CHANNEL_20MHZ_LOW_40MHZ_LOW:
			primary_channel = center_channel - 6;
			break;	
