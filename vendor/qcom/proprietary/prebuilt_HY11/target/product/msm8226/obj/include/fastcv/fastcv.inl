                        ...
                        int high )
{
   srcStride = (srcStride==0 ? srcWidth : srcStride);
   dstStride = (dstStride==0 ? srcWidth : dstStride);
   gradStride = (gradStride==0 ? 2*srcWidth : gradStride);
   ...
   return (**ppfcvFilterCanny3x3u8_v3)( src, srcWidth, srcHeight, srcStride, dst, dstStride, gx, gy, gradStride, low, high );
}
                           ...
                           int32_t                   threshold)
{
   ...
   (**ppfcvCornerHarrisAdaptiveu8)( src, srcWidth, srcHeight, srcStride, border, xy,
                        nCornersMax, nCorners, threshold );
}
                           ...
                           int16_t* __restrict dst )
{
   ...
   (**ppfcvFilterCorrSep13x13s16_v3)( knl, src, w, h, w*sizeof(int16_t), tmp, dst, w*sizeof(int16_t));
}
...
inline void
fcvFilterCorrSepNxNs16( const int16_t* __restrict kernel, int knlSize,
                        const int16_t* __restrict srcImg,
                        unsigned int w, unsigned int h, unsigned int srcStride,
                        int16_t* __restrict tmpImg,
                        int16_t* __restrict dstImg, unsigned int dstStride )
{
   ...
   return (**ppfcvFilterCorrSepNxNs16) (kernel, knlSize, srcImg, w, h, srcStride, tmpImg, dstImg, dstStride );
}
                      ...
                      uint32_t* __restrict dst )
{
   ...
   fcvAssert( ((int)dst & 0xF) == 0 );     // 128-bit alignment
   fcvAssert( (width & 0x7) == 0 );        // multiple of 8
   fcvAssert( ((uint64_t)height)*width < 16843009LL );
#endif
                        ...
                        unsigned int dstStride )
{
   ...
   fcvAssert( ((uint64_t)srcHeight)*srcWidth < 16843009LL );
#endif
...
fcvMserExtu8_v3 ( void *mserHandle,
                ...
                uint32_t* __restrict contourNodeCounter)
{
   return (**ppfcvMserExtu8_v3)(mserHandle, srcPtr, srcWidth, srcHeight, srcStride,
                                 maxContours, numContours, recArray, staPointsInPath, numPointsInContour,
                                 pathArraySize, pathArray, contourVariation, contourPolarity,
                                 contourNodeId, contourNodeCounter);
}
